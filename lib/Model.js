...
Model.batchGet = async function (NewModel, keys, options, next) {
  ...
  const emitObjectBatchGetCalled = {
    'event': {'callback': next, options, keys},
    'actions': {
      'updateCallback' (cb) { next = cb; },
      'updateKeys' (newKeys) { keys = newKeys; },
      'updateOptions' (newOptions) { options = newOptions; }
    }
  };
  const shouldContinueBatchGetCalled = await NewModel._emit('model:batchget', 'batchget:called', emitObjectBatchGetCalled, deferred);
  if (shouldContinueBatchGetCalled === false) { return deferred.promise.nodeify(next); }
  ...
  const batchGet = async () => {
    ...
    const emitObjectRequestPre = {
      'event': {options, batchReq},
      'actions': {'updateBatchReq' (newBatchReq) { batchReq = newBatchReq; }}
    };
    const shouldContinueRequestPre = await NewModel._emit('model:batchget', 'request:pre', emitObjectRequestPre, deferred);
    if (shouldContinueRequestPre === false) { return; }

    newModel$.base.ddb().batchGetItem(batchReq, async (err, data) => {
      const emitObjectRequestPost = {
        'event': {options, data, 'error': err},
        'actions': {
          'updateError' (newErr) { err = newErr; },
          'updateData' (newData) { err = newData; }
        }
      };
      const shouldContinueRequestPost = await NewModel._emit('model:batchget', 'request:post', emitObjectRequestPost, deferred);
      if (shouldContinueRequestPost === false) { return; }
